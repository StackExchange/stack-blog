...
During the original proposal stage for the engineering blog, we had a conversation about what engine we would use. At the time, all of our blogs were running WordPress...which we weren't so happy about. It was very buggy, difficult to log in to, not very performant, and has caused our SRE team more than a few headaches. If we were really going to revamp the new company blog, it seemed like a lot of work to try and wrestle with our WordPress installation.
...
We have a policy of being "default public" at Stack Exchange. That means we try and work in the open when we can, and that includes our technical work. Because the blog prototype I was building was already in GitHub, it seemed to make sense to just keep it publicly available. It was different from the work I was used to - the inclusion of issues, pull requests, and other features available to those outside of our internal team changed the way I approached building the blog. There were a number of reasons:

 - Our blog was going to be used by a non-technical audience as well. Having a GUI for our community and marketing team to create posts and preview markdown without learning Git was great.
 - It meant we left the door open to let people outside of company make changes, fix bugs, and even contribute posts in the future.
 - There are few examples of major migration efforts on Jekyll out there, and we want to be one of them.
 - People could take the work we did and use it for themselves. There are insights in building that we want to give back to the community.
 - If there are bugs *you* can fix them too!

## Building the blog
Alright - enough about the ideation process. Now to the meaty technical parts. There was a *lot* of development work that went into this, so I'm only going to highlight some of the major pieces and insights we had moving into this:
...
Out of the box, most of what we need was there: a posting engine with markdown and code highlight support, JavaScript and CSS flexibility, and it was *fast*. Because all the hard work was being done up front, we were just serving static files and that was a huge performance boost over Jekyll. In just a few minutes, I had a basic working blog that was being hosted on GitHub Pages. We were already well on our way to what we had originally set out to do, but that was easiest part.

### Importing the old content
Moving all of our previous blog posts to the new solution was a hard requirement, and that meant taking all the content we had on our existing WordPress and converting that into files in Markdown was a hard requirement. This was not easy: we had over 700 blog posts over the history of the company, each with comments, static assets, and deeplinking that had to preserved. The process seemed pretty straightforward if not riddled with edge cases that needed to be resolved: WordPress has an export function that spits out a giant XML file (a whopping [30MB](https://github.com/StackExchange/blog/blob/master/blog.xml) of text) and we just had to use that file to convert that into markdown files.

Thankfully, there are a few libraries that makes this migration easier. [I tried](https://github.com/StackExchange/blog/commit/7aa2cfa3959f3a7830bb972013ef3c9bea0e79eb) of a few of the import methods that were recommended in the Jekyll documentation, but none of them seemed to do it cleanly. After a few more attempts, I finally stumbled upon [exitwp](https://github.com/thomasf/exitwp) and that seemed to work pretty well. You can see the [commit here](https://github.com/StackExchange/blog/commit/e43a0ad1b9e4474c58229912eeb2db7106f9997a) - it was over 5,000 different file changes with the initial import.

For the most part, this worked pretty well. I was surprised by how much was preserved and the fact that Jekyll supports arbitrary HTML in the markdown files helped a lot. There were a few things that were lost in the process though: some images with captions were misformatted, iframe embeds were lost for many of our podcasts, and the links referencing uploaded files in WordPress broke images across the board.

This is when I had to really intervene to do some processing. I started writing Python scripts to go through every post and repair a lot of the work done here. Here's the high level overview:

 - For HTML errors, much of this repair work was manual. We looked at the top 50 high trafficked posts and the 100 most recent posts and manually went through to make sure everything was formatted well. This was done with a lot of help.
 ...
 - We used Soundcloud embeds for all of our podcasts, and the vast majority of this was broken. I wrote a Python script that used a regular expression to find the relevant URL for the import and reinserted the code necessary to restore the player. You can see the code for this below. It's very similar to the code I wrote for the image import as well.
...
### Pagination
This was actually one of the most fascinating technical challenges I ran into. Like I mentioned before, we have over 700 posts on our blog, and that was only going to grow, probably at an even faster speed. It was simply unreasonable to leave out pagination support. Sadly, Jekyll only has limited support for pagination. It could do so reasonably well for *all* posts, but we needed more fine grain pagination that would also filter by author and tag. In addition to the lack of customization for pagination, using the native capability *really* slowed down the generation time. Pages were going through each post, each category, and each other file and tried to generate every permutation of those combinations. Our build time for all posts went from about 5 seconds all the way to several minutes, because generation was going on MxNxO time. There were simply no good solutions for this natively in Jekyll or that other people had solved.

So I ended trying something completely different. Instead of relying on Jekyll to statically generate each page up front, I decided to only static generate JSON with all the posts and their data, then lazy load that data via AJAX and filter using the paramters in that post. It meant only *one* file had to be generated instead of every permutation, there was more programmatic flexibility with JavaScript to filter, and we could inject pagination into any page for any set of posts we wanted.

Here's the `json/index.json` file that is generated by Jekyll and used to generate the overview pages on each channel:
...
For commenting moving forward, we would use Disqus with a blank slate. If they ever got back to us or helped us with an import, we would backfill it then. We simply couldn't keep delaying shipping the new blog on an external dependency.
...
In the end, I'm glad that there's finally a single destination for our engineering team to reach out to the technical community we love being a part of. You'll see much more content like this moving forward, and we'd love to hear your feedback. Until next time!
